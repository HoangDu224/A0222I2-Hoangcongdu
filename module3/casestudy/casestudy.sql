...
select k.ma_khach_hang , k.ho_ten , count(h.ma_hop_dong) as 'so lan thue' , l.ten_loai_khach
from khach_hang k join hop_dong h on k.ma_khach_hang = h.ma_khach_hang join loai_khach l on k.ma_loai_khach = l.ma_loai_khach
...
select k.ma_khach_hang, k.ho_ten, ifnull( h.ma_hop_dong,'Chưa từng đặt phòng') as 'ma_hop_dong', 
       lk.ten_loai_khach , h.ngay_lam_hop_dong , h.ngay_ket_thuc, dvdk.ten_dich_vu_di_kem,
...
-- của tất cả các loại dịch vụ chưa từng được khách hàng thực hiện đặt trong quý 1 của năm 2021 (Quý 1 là tháng 1, 2, 3).
select dv.ma_dich_vu , dv.ten_dich_vu, dv.dien_tich , dv.chi_phi_thue , ldv.ten_loai_dich_vu, ldv.ma_loai_dich_vu
from dich_vu dv join loai_dich_vu ldv on dv.ma_loai_dich_vu = ldv.ma_loai_dich_vu
where dv.ma_dich_vu not in (
select ma_dich_vu
from dich_vu join hop_dong hd using (ma_dich_vu)
...
select kh.ho_ten from khach_hang kh;
-- -----------------
select kh.ho_ten ,ROW_NUMBER() OVER(ORDER BY ho_ten) as 'stt'
from khach_hang kh
group by ho_ten;
-- -----------------
...
select month(hd.ngay_lam_hop_dong) as 'thang' , count(month(hd.ngay_lam_hop_dong)) as 'so khach hang'
from hop_dong hd
where year(hd.ngay_lam_hop_dong) = 2021
...
select hd.ma_hop_dong , nv.ho_ten as 'ho_ten_nhan_vien', kh.ho_ten as 'ho_ten_khach_hang', kh.so_dien_thoai, dv.ten_dich_vu , hd.tien_dat_coc, ifnull(sum(hdct.so_luong),0) as 'so_luong_dich_vu_di_kem'
from dich_vu dv
...
and not exists ( 
select month(hd.ngay_lam_hop_dong), year(hd.ngay_lam_hop_dong)
from hop_dong
...
group by dvdk.ma_dich_vu_di_kem
having so_luong_dich_vu >= all ( select so_luong from hop_dong_chi_tiet );
...
where nv.ma_nhan_vien not in (select ma_nhan_vien from hop_dong group by ma_nhan_vien having count(ma_hop_dong) >3);

-- 16.	Xóa những Nhân viên chưa từng lập được hợp đồng nào từ năm 2019 đến năm 2021.
select  nv.ma_nhan_vien , nv.ho_ten -- thay select = delete
from nhan_vien nv
left join hop_dong hd on hd.ma_nhan_vien = nv.ma_nhan_vien
where nv.ma_nhan_vien not in ( select ma_nhan_vien from hop_dong where year(ngay_lam_hop_dong) in (2019,2021));

-- 17.Cập nhật thông tin những khách hàng có ten_loai_khach từ Platinum lên Diamond, 
-- chỉ cập nhật những khách hàng đã từng đặt phòng với Tổng Tiền thanh toán trong năm 2021 là lớn hơn 10.000.000 VNĐ.
update loai_khach
set lk.loai_khach = 'Diamond'
